        ...
        for comment in comments:
            ...
            try:
                if (team_name + " Table").strip() == caption.text.strip():
                    if x == 0:
                        table_type = "Batting Stats"
                    else:
                        table_type = "Pitching Stats"
                        # get the table that has the caption
...
def get_teams_from_links(soup):
    ...
    if scorebox_div is not None:
        ...
        away_team_link = scorebox_div.find_all(
            "a", href=lambda href: href and "/teams/" in href and ".shtml" in href
        )[0]
...
def get_game_location(soup):
    ...
    for div in location_div:
        if "Venue" in div.text:
            location_div = div
    if location_div:
        return location_div.text.split("Venue:")[1].strip()
    else:
        return None
